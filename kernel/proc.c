...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      acquire(&p->lock);
      int tp = p->creationTime;
      int tp1 = p1->creationTime;
      if (p->state != RUNNABLE)
      {
        release(&p->lock);
        continue;
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (dp == processDp && p->numpscheduled < p2->numpscheduled)
      {
        flag1 = 1;
      }
      else
      ...
      if (dp == processDp && p->numpscheduled == p2->numpscheduled && p->creationTime < p2->creationTime)
      {
        flag2 = 1;
      }
      else
      ...
      else
      {
        ...
        else if (dp > processDp || flag1 || flag2)
        {
          if (p2)
            release(&p2->lock);
    ...
    for (np = proc; np < &proc[NPROC]; np++)
    {
      if (np->parent == p)
      {
        ...
        if (np->state == ZOMBIE)
        {
          ...
          *wtime = np->endTime - np->creationTime - np->totalRunTime;
